...
RSpec.describe RakeTerraform do
  it 'has a version number' do
    expect(RakeTerraform::VERSION).not_to be_nil
  end

  describe '.define_command_tasks' do
    context 'when instantiating RakeTerraform::TaskSets::All' do
      it 'passes the provided options and block' do
        ...
        allow(RakeTerraform::TaskSets::All).to(receive(:define))

        described_class.define_command_tasks(opts, &block)
  ...
  describe '.define_installation_tasks' do
    ...
    context 'when setting up tasks for terraform installation' do
      ...
      it 'uses a path of `pwd`/vendor/terraform by default' do
        ...
        expect(task_set.path)
          .to(eq("#{Dir.pwd}/vendor/terraform"))
      end
      ...
      it 'uses the supplied path when provided' do
        ...
        expect(task_set.path)
          .to(eq(File.join('tools', 'terraform')))
      end
    ...
    context 'when providers are supplied' do
      it 'defines dependency tasks for each provider' do
        ...
        expect(provider_task_sets.map(&:dependency))
          .to(eq(
                %w[
                  terraform-provider-something1
                  terraform-provider-something2
                ]
              ))
      end
      ...
      it 'adds provider ensure tasks as prerequisites for the terraform ' \
        ...
        expect(Rake::Task['terraform:ensure'].prerequisites)
          .to(include(
      ...
      it 'passes source and target binary name templates' do
        ...
        expect(
          [
            provider_task_sets[0].source_binary_name_template,
            provider_task_sets[0].target_binary_name_template
          ]
        ).to(eq(
               [
                 'terraform-provider-something1',
                 'terraform-provider-something1_v<%= @version %>'
               ]
             ))
      end
